...
// Damp Dry Signal on: cloud dampDryBeep:"DAMPDRYBEEP_ON" (rec[24] bit 0x40), the other bit of that byte.
...
describe('BDH_D30007_US', () => {
    ...
    test('the minute counters are big-endian, unlike the matching washer', () => {
        ...
        assert.equal(drying.energy, 3) // cloud: courseSpendPower 3 Wh
    })
    ...
    test('wrinkle care, the drum light and the damp dry signal share rec[24] without collision', () => {
        const wrinkle = feed([WRINKLE_CARE_ON])
        assert.equal(wrinkle.wrinkle_care, 'ON')
        assert.equal(wrinkle.damp_dry_signal, 'OFF')
        assert.equal(wrinkle.drum_light, 'OFF')

        const signal = feed([DAMP_DRY_SIGNAL_ON])
        assert.equal(signal.damp_dry_signal, 'ON')
        assert.equal(signal.wrinkle_care, 'OFF')
        assert.equal(signal.drum_light, 'OFF')
    })
    ...
    test('a remote-started cycle ends into the remote-maintain state, not plain End', () => {
        ...
        assert.equal(p.energy, 1818) // what the whole 195-minute Bedding cycle used
    ...
    test('powering off clears the cycle but not the energy it used', () => {
        ...
        assert.equal(p.energy, 10) // what the part-run had used
    })
