...
describe(MODEL_ID, () => {
    ...
    test('an unknown option is refused rather than guessed', () => {
        ...
        dev.setProperty('end_melody', 'Silent Night') // an LG melody name, but not one this model offers
    ...
    test('the error codes decode, and Water refill is not off by one', () => {
        for (const [code, want, error] of [
            ...
            thinq.emit('data', frame)
            const p = ha.devices[DEVICE_ID].properties
            assert.equal(p.error_message, want, `error ${code}`)
            assert.equal(p.error, error, `problem flag for error ${code}`)
        }
    })
