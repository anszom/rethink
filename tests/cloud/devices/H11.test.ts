...
function withoutWarnings(fn: () => void) {
...
describe('H11 Dishwasher', () => {
    ...
    test('discovery configuration follows the existing appliance entity conventions', () => {
        const { ha } = makeDevice()
        const components = ha.devices[DEVICE_ID].config!.components as Record<string, Record<string, unknown>>

        assert.equal(components.status.name, 'Status')
        assert.equal(components.status.icon, 'mdi:state-machine')
        ...
        assert.ok(!(components.status.options as string[]).includes('unknown'))

        assert.equal(components.course.icon, 'mdi:pin-outline')
        assert.ok(!(components.course.options as string[]).includes('unknown'))
        assert.equal(components.remaining_time.name, 'Remaining time')
        assert.equal(components.remaining_time.device_class, 'duration')
        assert.equal(components.initial_time.name, 'Initial cycle duration')
        assert.equal(components.initial_time.device_class, 'duration')
        assert.equal(components.energy.name, 'Cycle energy consumption')
        assert.equal(components.energy.icon, 'mdi:lightning-bolt')
        assert.equal(components.extra_dry.name, 'High heat drying active')
        assert.equal(components.high_temp.name, 'Sanitizing wash active')

        assert.equal(components.rinse_level.name, 'Rinse aid level')
        assert.equal(components.rinse_level.max, 4)
        assert.equal(components.extra_rinse.name, 'Extra rinse count')
        assert.equal(components.target_extra_rinse.name, 'Target extra rinse count')
        assert.equal(components.salt_level.max, 4)
        assert.equal(components.salt_level.name, 'Salt dispensing level')
        assert.equal(components.buzzer_level.name, 'Buzzer volume')
        assert.deepEqual(components.buzzer_level.options, ['Off', 'Low', 'High'])
        assert.equal(components.clean_reminder.name, 'Machine clean reminder')
        assert.equal(components.auto_dry.name, 'Auto dry')
        assert.deepEqual(components.remote_start_mode.options, ['Off', 'Permanent', 'One-time'])
        assert.equal(components.remote_start.name, 'Remote start enabled')
        assert.equal(components.target_delay.name, 'Start delay setting')
        assert.equal(components.target_delay.device_class, 'duration')
        assert.equal(components.target_delay.unit_of_measurement, 'h')
        assert.equal(components.target_high_temp.name, 'Target sanitizing wash')
        assert.equal(components.target_extra_dry.name, 'Target high heat drying')
        ...
        assert.deepEqual(
            Object.fromEntries(
                [
                    ...
                    'target_extra_dry',
                    'target_extra_rinse',
                ].map((key) => [key, components[key].unique_id]),
            ),
            {
                initial_time: '$deviceid-initial_cycle_duration',
                energy: '$deviceid-cycle_energy_consumption',
                extra_dry: '$deviceid-high_heat_drying',
                high_temp: '$deviceid-sanitizing_wash',
                extra_rinse: '$deviceid-extra_rinse_count',
                rinse_level: '$deviceid-rinse_aid_level',
                salt_level: '$deviceid-salt_dispensing_level',
                buzzer_level: '$deviceid-buzzer_volume',
                clean_reminder: '$deviceid-machine_clean_reminder',
                auto_dry: '$deviceid-auto_dry',
                delay_start: '$deviceid-delay_start_remaining',
                remote_start: '$deviceid-remote_start_enabled',
                target_delay: '$deviceid-start_delay_setting',
                target_high_temp: '$deviceid-target_sanitizing_wash',
                target_extra_dry: '$deviceid-target_high_heat_drying',
                target_extra_rinse: '$deviceid-target_extra_rinse_count',
            },
        )

        assert.ok(!components.state)
        assert.ok(!components.remain_time)
        assert.ok(!components.course_time)
        assert.ok(!components.energy_consumption)
    })
    ...
    test('0xEC processes only the fixed-offset current record from a real change frame', () => {
        ...
        assert.equal(properties.extra_rinse, 'Level 1')
    ...
    test('rejects unobserved lengths and malformed fixed record headers', () => {
        ...
        const formerSyntheticLength = Buffer.alloc(58)
        formerSyntheticLength[0] = 0xaa
        formerSyntheticLength[1] = formerSyntheticLength.length
        formerSyntheticLength[2] = 0x32
        formerSyntheticLength[3] = 0xec
        formerSyntheticLength[30] = 0x00
        formerSyntheticLength[31] = 0x18
        formerSyntheticLength[formerSyntheticLength.length - 1] = 0xbb
        thinq.emit('data', formerSyntheticLength)
    ...
    test('unknown enum values are published as unavailable instead of values outside their options', () => {
        const { ha, thinq } = makeDevice()
    ...
    test('does not send settings before a real status frame initializes the shared values', () => {
        ...
        withoutWarnings(() => dev.setProperty('rinse_level', '3'))
