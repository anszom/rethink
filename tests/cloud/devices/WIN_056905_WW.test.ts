...
describe(MODEL_ID, () => {
    ...
    test('Cool/fan-high values response publishes all expected HA properties', (t) => {
        ...
        // native unit is Celsius too (see WIN_056905_WW.ts), so these are published as-is.
    ...
    test('HA write climate-temperature converts to the wire Celsius*2 raw value, clamped to 16-30C', (t) => {
        const { ha, dev } = buildReadyDevice(t)

        ha.setProperty(DEVICE_ID, 'climate', 'temperature_command', '23.5')
        assert.equal(dev.raw_clip_state[0x1fe], 47)

        // Below the 16C floor clamps to 16C (raw 32).
        ha.setProperty(DEVICE_ID, 'climate', 'temperature_command', '10')
        assert.equal(dev.raw_clip_state[0x1fe], 32)

        // Above the 30C ceiling clamps to 30C (raw 60).
        ha.setProperty(DEVICE_ID, 'climate', 'temperature_command', '35')
