...
// A Bedding cycle started AT THE PANEL, never armed for remote start by hand, captured one second
// apart. The first frame is already Drying with rec[16] bit 0x01 clear; the second has it set. Nobody
// touched the machine in between — this is the appliance opening a remote-control session for itself
// once a run begins, which is why the bit is set in every Drying capture regardless of how the cycle
// was started. It is the single clearest frame pair for that claim.
const LIVE_PANEL_START = buf(
    'AA4030EC001B010100010007000403000100000040A800000000000000CA000000001B320100010007000403000100000000A800000001000000CA000000E3BB',
)
const LIVE_PANEL_START_ARMED = buf(
    'AA4030EC001B320100010007000403000100000000A800000001000000CA000000001B320100010007000403000100000000A900000001000000CA000000FCBB',
)
...
describe(MODEL_ID, () => {
    ...
    test('a panel start arms itself: the bit is clear as Drying begins and set one frame later', () => {
        const starting = feed([LIVE_PANEL_START])
        assert.equal(starting.status, 'Drying')
        assert.equal(starting.remote_start, 'OFF')
        assert.equal(starting.cycle, 'Bedding')

        const armed = feed([LIVE_PANEL_START_ARMED])
        assert.equal(armed.status, 'Drying')
        assert.equal(armed.remote_start, 'ON') // one second later, with nobody at the panel
    })
