...
describe('FAFXU25006', () => {
    ...
    test('the countdown falls while the cycle length stays put, and energy accumulates', () => {
        ...
        assert.equal(p.energy, 1) // cloud: courseSpendPower 1 Wh
    })
    ...
    test('0xEB single-record frames decode with the same offsets as 0xEC', () => {
        ...
        assert.equal(off.energy, 127) // the finished cycle's energy is still there
    ...
    test('the end of a cycle unlocks the door and then powers off', () => {
        ...
        assert.equal(ended.status, 'End')
        assert.equal(ended.energy, 144) // the whole cycle's energy
        ...
        assert.equal(off.energy, 144) // energy survives the power-off, it is not zeroed
    })
    ...
    test('a finished cycle reports End, then powers itself off', () => {
        ...
        assert.equal(off.energy, 83) // the energy total survives the power-off
    })
    ...
    test('the drum light is rec[37] bit 0x40', () => {
        ...
        assert.equal(off.energy, 1) // the other byte that moved in the same frame
