...
// rec[38]: door lock, matching doorLock. It was separated from the state by rec[22], which holds the
// state the appliance was in before the current one, in the same codes as rec[21] — the cloud's preState.
// That byte is not published, but the one frame where the cloud reported preState:"RUNNING" and
// doorLock:"ON" and nothing else had exactly two bytes move: rec[22] took the RUNNING code, which only a
// state field can, leaving rec[38] going 0 -> 1 as the door lock.
