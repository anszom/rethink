...
// rec[4:6] = [hour][minute], the projected TOTAL length of the run. It does not pin once the cycle
// starts: it tracks remaining + elapsed, recomputed live. Watched across a Sensor Dry bedding load, the
// machine revised the load upward mid-run and both this and the countdown jumped by the same 19 minutes
// in the same frame. The two halves round independently, so consecutive frames flutter by a minute
// (79/80/79). That relationship also cross-checks rec[19] as elapsed time in 15-second units: at every
// frame of that run, remaining + rec[19]/4 landed within a minute of this value.
...
// rec[18:20] is elapsed run time in 15-second units, high byte FIRST — the same most-significant-first
// convention as the [hour][minute] time fields. It advances by 4 a minute and freezes the instant a
// cycle is paused. rec[19] alone would cap at 63.75 minutes, which is why this went unpublished until a
// 79-minute bedding load crossed the boundary: rec[19] went 212 -> 21 while rec[18] stepped 0 -> 1, so
// the counter carries rather than wrapping. 16 bits covers 273 hours, and the reading is corroborated
// independently at every frame of both runs by initial_time - remaining_time.
//
// Still not published, now by choice rather than doubt: those two entities already exist, both update
// live, and their difference is this value. Documented here so the next person does not have to spend
// another 79-minute load working out whether the byte wraps.
//
