...
//
// Frames arrive in the usual AABB form - AA <8-bit total length> ... <chk8> BB.
// AABBDevice.processData strips AA/length and the trailing checksum and BB, so buf[0] is the class
// byte and the frame type sits at buf[1]:
//
...
const COOKTOPS: { key: string; name: string; offset: number }[] = [
    ...
    { key: 'cooktop_rear_center', name: 'Rear center element', offset: 72 },
]
...
export default class Device extends AABBDevice {
            ...
            allowExtendedType({
                ...
                components: {
                    ...
                    oven_target_temperature: {
                        platform: 'sensor',
                        unique_id: '$deviceid-oven_target_temperature',
                    ...
                    oven_door_lock: {
                        platform: 'binary_sensor',
                        unique_id: '$deviceid-oven_door_lock',
                        state_topic: '$this/oven_door_lock',
                        name: 'Oven door lock',
                    ...
                    temperature_adjustment: {
                        ...
                        // Deliberately no device_class: this is a difference, not an absolute
                        // temperature (see TEMPERATURE_ADJUSTMENT_OFFSET), and device_class
                        // 'temperature' would have HA rescale it as one - a +7°F trim would reach a
                        // metric installation as -13.9°C.
                        unit_of_measurement: '°F',
                        entity_category: 'diagnostic',
                    },
                    temperature_unit: {
    ...
    processStatus(rec: Buffer) {
        ...
        this.publishProperty('oven_target_temperature', temperature(rec, TARGET_TEMP_OFFSET, celsius))
        this.publishProperty('oven_temperature', temperature(rec, CURRENT_TEMP_OFFSET, celsius))
        ...
        this.publishProperty('oven_door', rec[FLAGS_OFFSET] & FLAG_DOOR_OPEN ? 'ON' : 'OFF')
        this.publishProperty('oven_door_lock', rec[FLAGS_OFFSET] & FLAG_DOOR_LOCKED ? 'ON' : 'OFF')
