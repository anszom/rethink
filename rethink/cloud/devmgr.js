	...
	mqtt(topic, payload, client) {
		...
		if(topic === 'clip/message/devices/' + payload.did) {
			...
			if(payload.cmd === 'device_packet' && client.deviceObj && payload.did === client.deviceObj.id) {
				...
				if(buf.length >= 13 && buf[2] == 0x04 && buf[3] == 0x00 && buf[4] == 0x00 && buf[5] == 0x00 && buf[6] == 0x87 && buf[7] == 0x02 && buf[8] == 0x04
				...
				else if(buf.length >= 6 && buf[0] == 0xAA && buf[buf.length-1] == 0xBB) {
					try {
						const len = buf[1]
						if(buf.length == len && len >= 6) { // length field = total message length, min 6 bytes
							const deviceType = buf[2]
							const command = buf[3]
							// AA(0) LEN(1) TYPE(2) CMD(3) DATA(4..len-3) CHECKSUM(len-2) BB(len-1)
							...
							const tlvLike = [{ t: (deviceType << 8) | command, v: value }]
							client.deviceObj.emit('data', tlvLike)
						}
					} catch(err) {
						console.log('Error parsing AA..BB frame:', err)
					}
				}
			}
		}
