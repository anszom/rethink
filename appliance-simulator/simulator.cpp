	...
	for(;;) {
		...
		if(nb == 0) {
			...
			if(nb == 11 + buf[8]) {
				...
				unsigned short rv = crc16_ccitt(buf, nb);
				printf(" crc=%02x\n", rv);

				if(rv == 0) {
					if(nb == 13 && !memcmp(buf+4, "\x65\x01\x01\x01\x02\xaa\x43", 7)) {
						tx(fd, 0x01, 0x10, 0x01, NULL, 0);
						//(const unsigned char*)"\x87\x01\x10\x01\x00", 5);
					}

					if(nb == 13 && !memcmp(buf+4, "\x65\x02\x02\x01\x02\x7d\x41", 7)) {
						...
						tx(fd, 0x02, 0x01, 0x9f, resp, sizeof(resp));
					}
					
					if(nb == 11 && !memcmp(buf+4, "\x65\x02\x10", 3)) {
						tx(fd, 0x02, 0x04, buf[7], (const unsigned char*)"\xca\xd0\x7f", 3);
					}
				}
				nb = 0;
			}
