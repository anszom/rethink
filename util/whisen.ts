...
function members(fields: Record<string, string>): string {
...
export function deviceInfoBody(nation: string, regionalCode: string): string {
    return members({ Nation: nation, regionalCode }) + '\r\n'
}
...
export function deviceConfigBody(
    ...
    const fields: Record<string, string> = { HomeApSSID: ssid, HomeApPW: password }
    if (keyType) fields.HomeApKeyType = keyType
    fields.TimeZone = tz
